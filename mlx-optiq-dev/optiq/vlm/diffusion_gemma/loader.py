...
def _gather_weights(model_dir: str) -> dict:
    ...
    # The vision/audio towers ride in a bf16 sidecar (OptiQ's layout for every VLM),
    # so the shards above hold only the language tower. Merge the sidecar back in —
    # it is deliberately outside the `model*.safetensors` glob, which is what keeps
    # third-party loaders from tripping over it.
    from ..sidecar import VISION_SIDECAR_NAME, resolve_vision_sidecar

    sidecar = resolve_vision_sidecar(model_dir)
    if sidecar is not None:
        vision = mx.load(str(sidecar))
        # build_vision_sidecar stores keys with only the leading `model.` stripped,
        # so DiffusionGemma's towers land as `encoder.vision_tower.…`. Put it back.
        for k, v in vision.items():
            weights.setdefault(k if k.startswith("model.") else f"model.{k}", v)
    return weights
