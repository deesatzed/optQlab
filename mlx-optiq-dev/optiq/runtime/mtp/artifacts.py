...
def resolve_model_dir(model_dir: Path | str) -> Path:
    """Resolve a model reference to a local directory.

    If ``model_dir`` already points at a local directory that contains a
    ``config.json``, return it unchanged. Otherwise treat it as a
    HuggingFace repo-id and resolve it to the cached snapshot directory via
    ``snapshot_download`` — a no-op fetch when the repo is already cached,
    which is the common case under ``optiq serve`` (mlx-lm has already
    downloaded the weights before the MTP engine attaches).
    """
    path = Path(model_dir)
...
def expected_mtp_file(model_dir: Path | str, config: dict[str, Any] | None = None) -> Path:
    ...
    for rel in ("optiq/mtp.safetensors", "mtp.safetensors",
                "mtp/weights.safetensors", "model-mtp.safetensors"):
...
def _inspect_hf_model(repo_id: str) -> ModelInspection:
    ...
    from optiq.runtime.mtp.backends.registry import compatibility_for_inspection
